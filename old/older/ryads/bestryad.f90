...
if (testing=='test') then ! Если нужно быстро показать, что программа работает, то сюда можно заранее вписать данные
    ...
    stepen=(/2,1/); n=6
    chis(:,1)=(/2,-2,1/); znam(:,1)=(/3,1,0/)
    write(*,*) 'Введен ряд: ', '      (-2)^k*x^(2k+1)/k!'
    go to 88
endif
!---------------------------------------
write(*,*) '-----------------------'
...
write(*,*) '(a*k+b), a**(b*k), (a*k+b)!. Найдите для каждого множителя числа a и b'
...
case(1,-1); if (zn==-1) ch=-ch ! Если знаменатель равен -1, то следует сделать его единичным, а числитель умножить на -1
    select case(ch)
...
case default; if (zn<0) then; ch=-ch; zn=-zn; endif ! Если знаменатель отрицательный, то умножим и его, и числитель на -1
...
do i=1,size(mass,dim=2)
    ...
    write(*,*) 'Если следующий множитель вида a^(b*k), введите "2"'
    ...
    case(2); write(*,*) 'Множитель a^(b*k). Введите a и b'
        read(*,*) mass(1,i), mass(2,i)
...
do j=1,size(mass,dim=2) ! Умножаются все множители
    ...
    case(2); res=res*(mass(1,j)**(mass(2,j)*i))
...
!---------------------------------------
subroutine input(chis,znam,stepen)
! *** Процедура выписывает общий член ряда в общем виде
implicit none
integer, parameter :: mp=8
integer(mp), dimension(0:,1:), intent(in) :: chis, znam
integer, dimension(1:2), intent(in) :: stepen
integer :: i
character(600) :: fch, fzn
!---------------------------------------
call formating(chis,fch); call formating(znam,fzn)
write(*,*) '   Введен ряд:'
write(*,'(4x'//fch//'" * x^(",i1,"k+",i1,") / "'//fzn//',1x)') chis(1:2,:), stepen(1:2), znam(1:2,:)
end subroutine input

subroutine formating(mass,fxx)
! Процедура является дополнением к процедуре input
implicit none
integer, parameter :: mp=8
integer(mp), dimension(0:,1:), intent(in) :: mass
integer :: i, ncifr
character(50) :: fa, fb
character(600), intent(out) :: fxx

fxx=''
!---------------------------------------
do i=1,size(mass,dim=2)
    call kolvocifr(mass(1,i),ncifr); write(fa,*) ncifr !Количество цифр, отводимых под a
    call kolvocifr(mass(2,i),ncifr); write(fb,*) ncifr ! -------"------- b
    select case(mass(0,i)) ! Написание зависит от типа множителя
    case(1); write(fxx,*) trim(fxx)//',"(",i'//fa//',"k+",i'//fb//',")"'
    case(2); write(fxx,*) trim(fxx)//',"(",i'//fa//',")^(",i'//fb//',"k)"'
    case(3); write(fxx,*) trim(fxx)//',"(",i'//fa//',"k+",i'//fb//',")!"'
    end select
enddo
end subroutine formating
